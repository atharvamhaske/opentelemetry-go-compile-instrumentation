...
func TestMuxServer(t *testing.T) {
	t.Parallel()
	testutil.Build(t, "", "muxserver", "go", "build", "-a")
	...
	testCases := []struct {
		...
		{
			name:       "subrouter route uses the full path template",
			method:     http.MethodGet,
			path:       "/api/apps/42",
			wantStatus: http.StatusOK,
			assertSpan: func(t *testing.T, span ptrace.Span) {
				assert.Equal(t, "GET /api/apps/{appId}", span.Name(),
					"subrouter span name must use the full path template")
				testutil.RequireAttribute(t, span, string(semconv.HTTPRouteKey), "/api/apps/{appId}")
			},
		},
		{
		...
		},
		{
			...
			assertSpan: func(t *testing.T, span ptrace.Span) {
				assert.Equal(t, "GET", span.Name(),
					"span name must remain plain method when no mux route matches")

				_, hasRoute := testutil.Attrs(span)[string(semconv.HTTPRouteKey)]
				assert.False(t, hasRoute,
					"http.route must not be set when no mux route matches")
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			f.Start("muxserver", fmt.Sprintf("-port=%d", port))
