...
func TestStreamingReader_OnDone(t *testing.T) {
	...
	called := false
	reader := NewStreamingReader(body, span, time.Now(), OpChat, false, 0, func() {
		called = true
	})
...
func TestStreamingReader_AbortOnReadError(t *testing.T) {
	...
	tp := sdktrace.NewTracerProvider(sdktrace.WithSpanProcessor(sr))
	tr := tp.Tracer("test")
	_, span := tr.Start(t.Context(), "test-abort-read-error")
...
func TestStreamingReader_AbortOnCloseBeforeCompletion(t *testing.T) {
	...
	tp := sdktrace.NewTracerProvider(sdktrace.WithSpanProcessor(sr))
	tr := tp.Tracer("test")
	_, span := tr.Start(t.Context(), "test-abort-close")
...
func TestStreamingReader_CloseAfterFinishReasonIsNormal(t *testing.T) {
	...
	tp := sdktrace.NewTracerProvider(sdktrace.WithSpanProcessor(sr))
	tr := tp.Tracer("test")
	_, span := tr.Start(t.Context(), "test-close-after-finish")
...
func TestStreamingReader_TruncatedAfterFinishReasonIsError(t *testing.T) {
	...
	tp := sdktrace.NewTracerProvider(sdktrace.WithSpanProcessor(sr))
	tr := tp.Tracer("test")
	_, span := tr.Start(t.Context(), "test-truncated-after-finish")
